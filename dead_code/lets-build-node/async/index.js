...
*/
const { loadSingleBuildFile } = require('./load');


const main = async (buildFile) => {
  // 1) Loading phase.
  const rules = await load(buildFile);
  console.log('rules:\n', rules);
  // 2) Analysis phase.
...
const load = async (buildFile) => {
  /* TODO(laat):
  Load external build-file dependencies when discovered.

  Should be event-based, but a while loop suffices.
  */
  console.log(await loadSingleBuildFile(buildFile));
  return await loadSingleBuildFile(buildFile)
};
